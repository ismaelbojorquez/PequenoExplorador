...
El repositorio contiene una foundation Unity mínima: URP, escena temporal `Bootstrap`, nueve assemblies con fronteras verificables, BuildTools, tests EditMode/PlayMode, smoke Android y checks CI opt-in. No contiene gameplay, save, Addressables, IAP, ads, backend ni arte final.
...
2. Leer [`docs/MVP_SCOPE.md`](docs/MVP_SCOPE.md), [`docs/02_TECHNICAL_ARCHITECTURE.md`](docs/02_TECHNICAL_ARCHITECTURE.md) y el alcance de Fase 06 en [`docs/ROADMAP.md`](docs/ROADMAP.md).
3. Reejecutar `scripts/validate`; no heredar el PASS de esta fase.
4. Mantener shell landscape, navegación y safe areas detrás de Presentation/Application; Bootstrap solo compone. La comparación tap-to-move queda en F08 antes de fijar input.

...
Save EditMode cubre default/round-trip, JSON determinista, atomicidad/checksum/backup, v0→…→v6, migraciones v4→v5→v6, merge de records, photos metadata, future schema read-only, cancelación y coalescing. PlayMode recrea el servicio y valida discovery/foto a través de recarga de Selva. No se usa `PlayerPrefs`, red, reloj real ni rutas versionadas. Detalle: [`10_SAVE_SYSTEM.md`](10_SAVE_SYSTEM.md).
...
Localización EditMode cubre español default, resolución, Smart variables/plurales, persistencia y restauración sin `PlayerPrefs`, fallback Development/Release y pseudo no persistible. PlayMode cambia ES/EN/pseudo sin reinicio, confirma refresh/persistencia y layouts a `1280×720`/`1920×1080`. El validator cubre 56 keys públicas, cinco colecciones, dos locales y glifos. Contrato: [`17_LOCALIZATION.md`](17_LOCALIZATION.md).
...
Discovery EditMode cubre first/repeat, grant idempotente, missing, Draft, retirado, denominadores y día agregado. Fotografía añade thresholds/scoring, best-photo, filenames/budgets, storage fallback, cancelación y shutter concurrente. PlayMode entra por interacción, prueba inválido/válido, spam, pausa, failure injector, reduce motion, recarga/repetición y unload con `ActiveTemporaryResources=0`; registra peak estimado/delta Editor, no device profiling. Economy/álbum permanecen fuera.

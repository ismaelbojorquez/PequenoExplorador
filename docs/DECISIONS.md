...
| T-019 | Save nació en schema v1 con `JsonUtility` builtin, SHA-256, DTOs Infrastructure, replace atómico/backup y future schema read-only; T-021/T-022/T-028/T-029 lo llevaron a v5. | Sin dependencia nueva, compatible con IL2CPP, conserva corruptos/futuros y no guarda PII. | Todo schema/serializer nuevo exige migración; cloud/cifrado/entitlements requieren ADR. |
...
| T-021 | Localization `1.5.12` + AndroidJNI builtin `1.0.0`; ES startup, EN completo y pseudo Development. `LocalizedKey` gobierna copy y Save persiste solo ES/EN (actualmente v5). | Versiones oficiales estables verificadas 2026-08-16; Smart Strings/fallback/asset tables evitan copy disperso y conservan offline/IL2CPP. | Actualizar paquete/locale/serializer exige intake, migración, validator, ES/EN/pseudo, permisos, IL2CPP y APK. |
...
| T-024 | IDs de contenido son value objects namespaced; ScriptableObjects compilan a definitions readonly e `IContentCatalog` O(1). Release acepta solo Approved no-placeholder; retiros usan aliases/migración. | Evita GUID/enums/switches y `AssetDatabase` runtime. `VS-D-A01` es la primera instancia Approved. | Cambiar ID publicado exige compatibilidad/migración; nuevo contenido exige fuentes, derechos y Gate B. |
...
| T-027 | Interacción contextual usa `InteractionId` + definition readonly, selector `prioridad→distancia→ID`, coordinador Application y adapters indexados. | Las categorías comparten núcleo sin hardcode animal; el tucán real y planta/objeto PH usan el mismo contrato. | Cambiar prioridad/rango/copy/cues requiere datos, overlap/touch y revisión infantil. |
...
| H-007 | Aprobación factual y responsables. H-007/H-008/H-009 de Ismael Bojórquez cubren Product/Education/Localization, visual/rights/QA y revisión factual declarada para `VS-D-A01`; no equivalen a credencial ornitológica externa ni publicación. | Revalidar con los mismos owners si cambia claim/asset; revisión independiente recomendada antes de Release. | Cumplida para el Vertical Slice; continua por cambio. |

...
Prompt 25 añade `PH_UI_CAMP_HUB`: cuatro estaciones grandes, preview modal con confirmación/cancelación y feedback localizado. Expedición/Álbum/Personalización están activos; Personas adultas sigue futura/no interactiva. `Back` cierra personalización o cancela preview antes de abandonar otro estado. Targets `≥64×64`, safe area y `4:3`/`16:9`/`20:9` se validan; no existe CTA de dinero real. Contratos: [`CAMP_SYSTEM.md`](CAMP_SYSTEM.md) y [`CUSTOMIZATION_SYSTEM.md`](CUSTOMIZATION_SYSTEM.md).

Prompt 26 añade `PH_UI_CUSTOMIZATION`: ocho botones de slot, hasta cuatro cards reutilizables por slot, swatch+texto, saldo, estado, unlock, equip y volver. Preview nunca compra/equipa; unlock y equip son acciones separadas. No hay pestañas o etiquetas por género. ES/EN y el layout 4:3/16:9/20:9 están automatizados; iconografía/TMP/arte y prueba infantil física quedan para Prompt 27/Gate C.

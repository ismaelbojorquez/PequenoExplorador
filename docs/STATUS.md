...
Actualizado: 2026-08-16 21:45 (`America/Mexico_City`). Git, implementación y evidencia ejecutada prevalecen si contradicen este resumen.

## Fase y Gate

- **Fase implementada más reciente:** Prompt 18 — discovery persistente e idempotente completado y validado.
- **Preparación editorial H-007:** [`VS-D-A01`](VS_D_A01_TOUCAN_FACTUAL_DOSSIER.md) en `Sourced + Owner/Product Approved`; Ismael Bojórquez aprobó Product/Education/Localization y declaró Art/Audio/Rights/QA como Creador. Especialista factual y assets verificables siguen pendientes; runtime permanece Draft/PH_.
...
- **Siguiente fase:** completar especialista factual y producir/mapping de asset no-`PH_`; después Prompt 19. No iniciar fotografía mientras el único fixture siga Draft.
...
| Pipeline local | `PASS` | Baseline antes del registro humano: `scripts/validate` código `0`; checks, compile/validadores, Addressables, EditMode `103/103`, PlayMode `18/18` y APK. Los cambios posteriores son solo Markdown y pasaron checks documentales. |
...
| Expediente factual VS-D-A01 | Owner/Product aprobado; gates técnicos `BLOCKED` | Ismael Bojórquez aprobó claims/copy y eligió `Tucán pico canoa`; conservación excluida. Competencia declarada `Creador`, no especialista factual. Art/audio/rights `Propia` carecen aún de asset ledger; runtime no cambió. |
...
| Tests Unity | `PASS` | EditMode `103/103`, `2.119 s`; PlayMode `18/18`, `12.186 s`. Añade discovery first/repeat/idempotencia/queries/migración y persistencia por reload Selva. |
| Android | Development `PASS`; Release `BLOCKED` | Baseline H-007 produjo APK ES `105,942,573` bytes, SHA-256 `8a45e5919a06a4d8a7f467dcf82c9f5ddd117054d95ad0b0be637bcf6bdfc095`, `15.820 s` Unity, API 26/36, IL2CPP/ARM64 y zipalign 16 KB. Es foundation, no Vertical Slice; sin permiso CAMERA/sensible nuevo. |
...
- Prompt 19 exige un animal fixture Approved, pero el único discovery/animal runtime sigue neutral Draft. Product/Education y nombre regional ya tienen aprobación del Creador; faltan especialista factual y assets no-`PH_` con evidencia/QA efectiva.
...
Obtener revisión de especialista factual para [`VS-D-A01`](VS_D_A01_TOUCAN_FACTUAL_DOSSIER.md), crear assets no-placeholder propios con ID/source/export/license ledger y validator/aliases aplicables, y solo entonces ejecutar el preflight de Prompt 19. No marcar Draft como Approved, introducir contenido masivo, cámara física/permisos, segundo mundo, remote, signing, push o publicación.

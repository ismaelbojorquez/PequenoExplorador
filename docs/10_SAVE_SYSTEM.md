...
Estado: schema v1 implementado en Prompt/Fase 09. Es foundation técnica sin gameplay, cloud, cuentas ni entitlements.
...
Infrastructure.LocalSaveService
        ├─ UnityJsonSaveSerializer → SaveEnvelope + DTO v1
...
```json
{
  ...
  "payload": "json-escapado-del-dto-v1"
}
```

El payload v1 contiene:
...
| `settings` | Guía, música, SFX y narración; no edad. | No. |
...
1. Sin primary/backup: crear `PlayerProgress` default y escribir schema v1.
2. Primary v1 válido: validar checksum, DTO/invariantes y cargar.
3. Primary antiguo: aplicar registro ordenado `n→n+1`; F09 incluye fixture/migración real `v0→v1`; reescribir v1 y conservar el original como backup.
...
| Caso | Evidencia |
|---|---|
| Default v1 y round-trip | EditMode. |
...
| v0→v1 y migración ausente | Migración/backup o fallo conservador sin rewrite. |

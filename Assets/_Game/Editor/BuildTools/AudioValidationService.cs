...
namespace PequenoExplorador.Editor.BuildTools
{
    public static class AudioValidationService
    {
        private static readonly HashSet<string> ExpectedIds = new HashSet<string>(StringComparer.Ordinal)
        {
            ...
            "audio.voice.tutorial.enter-expedition", "audio.voice.tutorial.move", "audio.voice.tutorial.interact",
            "audio.voice.tutorial.photograph", "audio.voice.tutorial.discovery-reward", "audio.voice.tutorial.return-camp",
            "audio.voice.tutorial.open-album"
        };
        ...
        public static IReadOnlyList<string> Validate()
        {
            ...
            if (cues.Length != 16 || !ExpectedIds.SetEquals(cues.Select(cue => cue.RawCueId)))
                violations.Add("AUDIO004 catalog must contain the sixteen stable cue IDs exactly once");
            ...
            if (audioFiles.Length != 28 || audioFiles.Any(path => !path.StartsWith(AudioFoundationSetup.ClipRoot + "/PH_", StringComparison.Ordinal)))
                violations.Add("AUDIO006 only twenty-eight PH_ clips may exist under the audio placeholder root");
